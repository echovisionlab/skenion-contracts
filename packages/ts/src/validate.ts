...
function releaseTrainRegistryPackageGateErrors(manifest: ReleaseTrainManifestV01): string[] {
  return releaseTrainRegistryPackageGatePackages(manifest)
    ...
    .map(([label]) => `release-gates.registry-packages.${label}.package must match component package`);
}
...
function releaseTrainDesktopArtifactNameErrors(
  ...
  for (const [target, artifact] of Object.entries(artifacts)) {
    ...
    if (artifact.source.kind === "github-release-asset" && artifact.source["asset-name"] !== expectedName) {
      errors.push(`${label} ${target} asset-name must be ${expectedName}`);
    }
  }
...
function releaseTrainStudioWebBundleArtifactErrors(
  ...
  if (artifact.source.kind !== "github-release-asset") {
    ...
    if (artifact.source["asset-name"] !== expectedName) {
      errors.push(`${label}.source.asset-name must be ${expectedName}`);
    }
  }
...
function releaseTrainRuntimeSmokeGateErrors(
  ...
  for (const target of releaseTrainTargetsV01) {
    ...
    errors.push(...releaseTrainArtifactIdErrors(artifactsById, "runtime-smoke", gate["artifact-id"]));
    if (gate.target !== target) {
      errors.push(`runtime-smoke ${target} target must match map key`);
    }
    if (gate["artifact-id"] !== artifact.id) {
      errors.push(`runtime-smoke ${target} artifact-id must match runtime binary`);
    }
  }
...
function releaseTrainStudioSmokeGateErrors(
  ...
  for (const target of releaseTrainTargetsV01) {
    ...
    if (gate.target !== target) {
      errors.push(`studio-package-smoke ${target} target must match map key`);
    }
    if (gate["desktop-package-artifact-id"] !== desktopPackage.id) {
      errors.push(`studio-package-smoke ${target} desktop-package-artifact-id must match desktop package`);
    }
    if (gate["runtime-sidecar-artifact-id"] !== runtimeSidecar.id) {
      errors.push(`studio-package-smoke ${target} runtime-sidecar-artifact-id must match runtime sidecar`);
    }
  }
...
function validateReleaseTrainManifestV01Semantics(manifest: ReleaseTrainManifestV01): string[] {
  ...
  if (!manifest["train-version"].startsWith(`${manifest["train-id"]}.`)) {
    errors.push("train-version must match train-id major.minor");
  }

  for (const [label, packageRef] of releaseTrainPackageVersions(manifest)) {
    if (packageRef.version !== manifest["train-version"]) {
      errors.push(`${label} version must be ${manifest["train-version"]}`);
    }
  }
  ...
  errors.push(
    ...
    ...releaseTrainArtifactVersionErrors(
      ...
      "runtime binary",
      manifest["train-version"]
    ),
    ...releaseTrainArtifactVersionErrors(
      manifest.components.studio["desktop-packages"],
      "studio desktop package",
      manifest["train-version"]
    ),
    ...releaseTrainArtifactVersionErrors(
      manifest.components.studio["runtime-sidecars"],
  ...
  if (manifest.components.examples.version !== manifest["train-version"]) {
    errors.push(`examples version must be ${manifest["train-version"]}`);
  }
  ...
  if (manifest.components.docs.manual.version !== manifest["train-version"]) {
    errors.push(`docs manual version must be ${manifest["train-version"]}`);
  }
  ...
  if (manifest["release-gates"]["github-release-assets"].runtime.repository !== releaseTrainRuntimeRepositoryV01) {
    errors.push(`github-release-assets runtime repository must be ${releaseTrainRuntimeRepositoryV01}`);
  }
  if (manifest["release-gates"]["github-release-assets"].studio.repository !== releaseTrainStudioRepositoryV01) {
    errors.push(`github-release-assets studio repository must be ${releaseTrainStudioRepositoryV01}`);
  }
  errors.push(
  ...
  if (!manifest["release-gates"]["github-release-assets"].studio["artifact-ids"].includes(manifest.components.studio["web-bundle"].id)) {
    errors.push(`github-release-assets studio artifact-ids must include components.studio["web-bundle"].id`);
  }
  if (!manifest["release-gates"]["checksum-verification"]["artifact-ids"].includes(manifest.components.studio["web-bundle"].id)) {
    errors.push(`checksum-verification artifact-ids must include components.studio["web-bundle"].id`);
  }

  const expectedChecksums = manifest["release-gates"]["checksum-verification"]["expected-checksums"] ?? {};
